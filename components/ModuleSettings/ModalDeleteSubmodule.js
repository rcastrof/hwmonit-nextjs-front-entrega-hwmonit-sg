...
const ModalDeleteSubmodule = (props) => {
    ...
    const handleDelete = async () => {
        ...
        if(props.ide !== null && props.ide !== undefined){
            ...
            const listDetails = await new Promise((resolve, reject) => {
                axios.delete(`${process.env.REACT_APP_API_URL}deleteSubmodule/${props.ide}`, { withCredentials: true })
                    .then(response => {
                        resolve(response);
            ...
            if(listDetails.status===200){
                props.parentCallback({state: true, status: listDetails.status,  message: "deleteSubmodule"});
                    ...
                    >
                        <div className="inline-block w-[327px] h-[246px] align-bottom bg-[#D6E1E7]/25 bg-gradient-to-b from-[#003C61] to-[#1B6970] text-left  shadow-xl transform transition-all my-[6rem] m:my-[15rem] rounded-[1.875rem]">
                            ...
                            <div className="flex h-[120px] ml-[1rem] ">
                                ...
                                <div className='leading-[19.07px] tracking-[-5%] text-[20px] self-center text-[#FAFAFA] font-bold'>¿Seguro que deseas eliminar este submódulo?</div>
                            ...
                            <div className='flex mt-[1rem]'>
                                ...
                                <button
                                    className="h-[48px] w-[136px] bg-[#EA683F] rounded-[10px] ml-[24px] text-[16px] leading-[22px] tracking-[-1px] "
...
export default ModalDeleteSubmodule;