    ...
    const handleDelete = async () => {
       ...
       if(selection == 1){
        ...
        if(selection==2){
            ...
            const listDetails = await new Promise((resolve, reject) => {
                axios.delete(`${process.env.REACT_APP_API_URL}branchDelete/${branchId}`, { withCredentials: true })
                    .then(response => {
                        resolve(response)
                    ...
                    >
                        <div className="inline-block w-[327px] h-[246px] align-bottom bg-[#D6E1E7]/25 bg-gradient-to-b from-[#003C61] to-[#1B6970] text-left  shadow-xl transform transition-all my-[6rem] m:my-[15rem] rounded-[1.875rem]">
                            ...
                            <div className="flex h-[120px] ml-[1rem] ">
                                ...
                                <div className='leading-[19.07px] tracking-[-5%] text-[20px] self-center text-[#FAFAFA] font-bold'>  ¿Seguro que deseas eliminar {selection == 1 ? "este contacto?": null}
                                {selection == 2 ? "esta Sucursal?": null} </div>
