...
const ModalCreateNewPackage = (props) => {

    const { show, onClose, addNewPackage, isRepeat } = props;
    ...
    const onSubmit = async (event) => {
        ...
        if(isRepeat(packageName)){
            setError("package_name", {
                type: "repeat",
                message: "Nombre del paquete repetido"
            })
            return
        }
        ...
        if (saveForm) {
            ...
            setSaveForm(false);
            const package_name = event.package_name;
            const package_version = event.package_version;
            addNewPackage(package_name, package_version);
            reset({ package_name: "", package_version: "" });
            setSaveForm(true);
            closeModal()
        }
    };
                                    ...
                                    <label className='text-[#FAFAFA] text-sm flex flex-col'>
                                        <div className='overflow-auto gt-scroll h-[310px]'>
                                            ...
                                            <input
                                                ...
                                                autoComplete="off" placeholder="Ingresa el nombre del paquete" minLength={2} maxLength={30} autoFocus />

                                            {errors.package_name && <span className='text-xs text-[#FAFAFA] ml-[24px]'>{errors.package_name.message}</span>}



                                            <div className='mb-[8px] ml-[24px] font-bold leading-[19.07px] tracking-[-5%] text-[14px] mt-[18px]'>Versión</div>
                                            <input
                                                {...register("package_version", {
                                                    required: { value: true, message: "* Campo Requerido" },
                                                    minLength: { value: 2, message: "* Campo debe tener 2 o mas de longitud" },
                                                    maxLength: { value: 50, message: "* Longitud Máxima de 50 caracteres" },
                                                    pattern: { value: VersionRegex, message: "* Versión Inválida, Ejemplo versión: 12.3.53" }
                                                })} name="package_version" type="text"
                                                className="ml-[24px] border-[2px] border-white/[0.20] bg-transparent rounded-[9px] w-[279px] h-[48px] pl-[16px] focus:outline-none focus-visible:ring-1 focus-visible:ring-white"
                                                autoComplete="off" placeholder="Ingresa la versión del paquete" minLength={2} maxLength={30} />

                                            {errors.package_version && <span className='text-xs text-[#FAFAFA] ml-[24px]'>{errors.package_version.message}</span>}
...
export default ModalCreateNewPackage;